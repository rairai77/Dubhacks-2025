...
Tabs:
${tabsData.map(t => `ID: ${t.id}, Title: ${t.title}, URL: ${t.url}, Content: ${t.content}`).join('\n')}
...
async function performAIGrouping() {
    try {
        ...
        showStatus(`Successfully created ${groups.length} tab groups!`, 'success');
...
// Event listeners
document.getElementById('aiGroupTabs').addEventListener('click', performAIGrouping);
